...
void runMatrixExperimentSIMD(unsigned int min, unsigned int max, unsigned int repeats, const std::string &file_out) {
  ...
  for (unsigned int e = min; e < max; e++) {
    ...
    ss << std::setw(15) << (std::to_string(mat_rows) + ",") << std::flush;
    t.start();
    auto mat_a = Matrix<float>(mat_cols, mat_rows); 
    auto mat_b = Matrix<float>(mat_cols, mat_rows);  
    auto mat_c = Matrix<double>(mat_cols, mat_rows);
    auto mat_d = Matrix<double>(mat_cols, mat_rows);
    t.stop();
    t.report(ss);
    t.start();
    mat_a.randomize(2);
...
void runMatrixExperimentOMP(unsigned int min,
  ...
  std::cout << "OpenMP matrix multiplication experiment being implemented." << std::endl;
  std::ofstream fos(file_out);
  if (!fos.good()) {
    std::cerr << "Could not open file " + file_out << std::endl;
    std::exit(-1);
  ...
  for (unsigned int e = min; e < max; e++) {
    ...
    t.start();  
    auto mat_a = Matrix<float>(mat_cols, mat_rows);
    auto mat_b = Matrix<float>(mat_rows, mat_cols);
    auto mat_c = Matrix<double>(mat_cols, mat_rows);
    auto mat_d = Matrix<double>(mat_rows, mat_cols);
    t.stop();
    t.report(ss);
    t.start();
    mat_a.randomize();
